...
func (client *Client) Setnx(key string, val string) (bool, error) {
	res, err := client.sendCommand("SETNX", key, val)
...
func (client *Client) Setex(key string, time int64, val string) error {
	_, err := client.sendCommand("SETEX", key, strconv.FormatInt(time, 10), val)
...
func (client *Client) Rpush(key string, val string) error {
	_, err := client.sendCommand("RPUSH", key, val)
	if err != nil {
		return err
	}
...
func (client *Client) Lpush(key string, val string) error {
	_, err := client.sendCommand("LPUSH", key, val)
	if err != nil {
		return err
	}
